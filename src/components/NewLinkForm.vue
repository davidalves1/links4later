...
const onSubmit = () => {
  console.log('🚀 ~ onSubmit:', { url: url.value, description: description.value });
}
...
<template>
  <form @submit.prevent="onSubmit" class="flex flex-col bg-white p-3 rounded-md gap-3">
    ...
    <button
      type="submit"
      class="px-3 py-2 rounded hover:bg-opacity-90 transition duration-300 flex justify-center bg-cyan-900 text-white self-end min-w-24"
    >
      Add
    </button>
  </form>
